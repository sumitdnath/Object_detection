...
flags = tf.app.flags
flags.DEFINE_string('data_dir', '/home/wangshiyao/Documents/data/imagenet/VID/ILSVRC/', 'Root directory to raw VID dataset.')
...
def dict_to_tf_example(data,
  ...
  flow_path = os.path.join(dataset_directory, 'Data/VID', setname+'_flow', img_path+'.JPEG')
  ...
  with tf.gfile.GFile(flow_path, 'rb') as fid:
    flow_encoded_jpg = fid.read()
  flow_encoded_jpg_io = io.BytesIO(flow_encoded_jpg)
  flow_image = PIL.Image.open(flow_encoded_jpg_io)
  if flow_image.format != 'PNG':
    raise ValueError('Flow Image format not PNG')
  flow_key = hashlib.sha256(flow_encoded_jpg).hexdigest()

  width = int(data['size']['width'])
  height = int(data['size']['height'])
  print(image.size, flow_image.size, width, height, data['filename'] )
  ...
  if 'object' in data:
      for obj in data['object']:
        ...
        #print ("label_map_dict[obj['name']]", obj)
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/flow_key/sha256': dataset_util.bytes_feature(flow_key.encode('utf8')),
      'image/encoded': dataset_util.bytes_feature(encoded_jpg),
      'image/flow_encoded': dataset_util.bytes_feature(flow_encoded_jpg),
      'image/format': dataset_util.bytes_feature('jpeg'.encode('utf8')),
      'image/flow_format': dataset_util.bytes_feature('png'.encode('utf8')),
